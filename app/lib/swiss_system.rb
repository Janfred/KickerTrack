# Module containing all SwissSystem relevant classes
module SwissSystem
  ...
  class SwissSystem
    ...
    def initialize(teams, paarings, pointgroup_identifier, sort_function)
      ...
      @paarings.each do |paaring|
        @teams_by_id[paaring[:team_1]][:blacklist] << paaring[:team_2] if paaring[:team_1] && paaring[:team_2] && @teams_by_id[paaring[:team_1]]
        @teams_by_id[paaring[:team_2]][:blacklist] << paaring[:team_1] if paaring[:team_1] && paaring[:team_2] && @teams_by_id[paaring[:team_2]]
      end
    end

    # Calculates a basic paaring (only here as fallback)
    # @return an array of arrays containing the TeamIDs for all calculated paarings
    # @raise [SwissSystemPaaringException] if no paaring is possible
    def basic_paaring
      paarings = basic_paaring_recursive(@teams_by_id.keys, [])
    end

    # Calculate a new round
    # @param timeout maximum seconds of calculation after wich the calculation is aborted if no solution has been found. optional, 0 => no timeout
    # @return an array of arrays containing the TeamIDs for all calculated paarings
    # @raise [SwissSystemPaaringException] if no paaring is possible
    # @raise [SwissSystemTimeoutException] if the calculation time exceeds the timeout
    def swiss_system_paaring(timeout = 0)
    end
    ...
    def basic_paaring_recursive(team_ids, current_paarings)
      ...
      team_ids.each do |team2|
        ...
        next if @teams_by_id[team1][:team][:freeplay_lock] && @teams_by_id[team2][:team][:freeplay_team]
        begin
  ...
  #
  # Thrown when no paarings could be found in the given time
  class SwissSystemTimeoutException < RuntimeError
  end
end
